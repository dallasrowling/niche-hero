...
class niche_hero_recent_posts_with_featured_image extends WP_Widget {
    ...
    public function widget($args, $instance) {
		...
		if( $niche_hero_posts->have_posts() ) {
			echo '<ul class="niche-hero-widget-recent-posts-with-fi">';
            ...
            <li>
				<div class="nh-image-holder">
					...
					<?php if ( $show_video_views == true ): ?><span class="video-views"> <?php echo $nh_post_views; ?></span><?php endif; ?>
					<?php if ( $show_comments == true ): ?><span class="comment-number"> <?php echo get_comments_number($instance['ID']) ?></span> <?php endif; ?>
				</div>
				<div class="nh-meta-data">
					<h4><?php the_title(); ?></h4>
					<?php if ( $show_date == true ) : ?><span class="nh-date"><?php echo get_the_date(); ?></span><?php endif; ?>
					<a href="<?php the_permalink(); ?>" class="nh-read-more">Read more</a>
				</div>
            </li>
          <?php endwhile;
        }
    ...
    <p>
      <label for="<?php echo $this->get_field_id( 'number' ); ?>">How many posts would you like to display?</label>
      ...
      <label for="<?php echo $this->get_field_id( 'thumbsize' ); ?>">Size of the featured image?</label>
      <input id="<?php echo $this->get_field_id( 'thumbsize'); ?>" style="min-height: 30px; padding: 0 8px; width: 100%; border-radius: 4px; border: 1px solid #8c8f94;" name="<?php echo $this->get_field_name( 'thumbsize' ); ?>" value="<?php echo esc_attr($thumbsize); ?>" /><br><br>
      <?php if (is_plugin_active('post-views-counter/post-views-counter.php')) { ?>
		<label for="<?php echo $this->get_field_id( 'show_video_views' ); ?>">Show video views per post
		<input id="<?php echo $this->get_field_id( 'show_video_views'); ?>" name="<?php echo $this->get_field_name( 'show_video_views' ); ?>" <?php checked($instance['show_video_views'], true) ?>  type="checkbox" /></label><br><br>
	  <?php } ?>
      <label for="<?php echo $this->get_field_id( 'show_comments' ); ?>">Show amount of comments per post
      <input id="<?php echo $this->get_field_id( 'show_comments'); ?>" name="<?php echo $this->get_field_name( 'show_comments' ); ?>" <?php checked($instance['show_comments'], true) ?>  type="checkbox" /></label><br><br>
	  <label for="<?php echo $this->get_field_id( 'show_date' ); ?>">Show date per post
